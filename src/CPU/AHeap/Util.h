...
// Find top-k flows in my_map and store them in my_set
...
// Find  heavy hitters in my_map and store them in my_set
...
// Test accuracy with tolerance of deviation 
...
// Merge a flow's all counters stored in differen levels of the AHeap 
void MergeSingleCache(unordered_map<string, int> &un_map, AHeap &my_cache){
...
// Merge a flow's all counters stored in differen levels of the AHeap 
void MergeSingleCache(unordered_map<string, vector<MyItem>> &un_map, AHeap &my_cache ){
    for(int i = 0; i < my_cache.bucket_num; i++){
        uint32_t now_count = my_cache.MyCache[i].my_value_count;
        if(now_count != 0){
            string key(my_cache.MyCache[i].my_key, my_cache.key_len);
...
// Sort items according to their counters from large to small
...
// Insert an item into one of AHeaps with specific key length
void AHeapInsert(AHeap& MyCache, int s_id, char* key, int key_len, int k, int p, uint32_t time_s, int full_len){
    char *full_key;
...
// Insert an item into one of AHeaps with specific key length considering the thresholds of time window and value of counter
void AHeapInsert_time_aging(AHeap& MyCache, int s_id, char* key, int key_len, int k, int p, uint32_t time_s, int full_len, int time_thre, int counter_thre){
    char *full_key;
    full_key = new char[full_len];
    memcpy(full_key, key, key_len);
    int key_index = 0;
...
// Insert an item while performing flowlet finding 
void AHeapInsert_for_flowlet(AHeap& MyCache, int s_id, char* key, int key_len, int k, int p, uint32_t time_s, int full_len){
    char *full_key;
    full_key = new char[full_len];
    memcpy(full_key, key, key_len);
    int key_index = 0;
...
// Insert an item while performing recent flow finding
void AHeapInsert_for_recent(AHeap& MyCache, int s_id, char* key, int key_len, int k, int p, uint32_t time_s, int full_len){
    char *full_key;
    full_key = new char[full_len];
    memcpy(full_key, key, key_len);
    int key_index = 0;
...
// Insert an item while performing burst finding
void AHeapInsert_for_burst(AHeap& MyCache, int s_id, char* key, int key_len, int k, int p, uint32_t time_s, int full_len){
    char *full_key;
    full_key = new char[full_len];
    memcpy(full_key, key, key_len);
    int key_index = 0;
...
bool comp_cache(const AHeap &a, const AHeap &b){
    return a.Cacheid < b.Cacheid;
}


//Decode all potential keys in caches, and replace the segments with keys, results are saved in un_maps.
void _DecodeCaches(vector<AHeap> &un_maps, vector<AHeap> &caches, vector<int> &paras){
    int k = paras[0];           // Number of source fragments. e.g. 3
    int p = paras[1];           // Number of parity fragments. e.g. 2
    int actual_len = paras[2];  // Length of key. e.g. 13
    int full_len = paras[3];    // Length of decoded key. e.g. 15
    int level_num = paras[4];   // Total level number of a cache. e.g. 4

    // initial start place of each level.
    ...
    for(int i = 0; i < caches.size(); i++){
        AHeap tmp(caches[i].min_bucket_num, caches[i].item_size, caches[i].Cacheid);
...
// Get statistics of flows by decoding with partial keys from different cache
void DecodeCaches(vector<pair<unordered_map<string, vector<MyItem>>, int>> &un_maps, vector<AHeap> &caches, vector<int> &paras){
    un_maps.clear();
    vector<AHeap> ret_caches; ret_caches.clear();
