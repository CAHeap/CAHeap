...
void MergeSingleCache(unordered_map<string, int> &un_map, FlowCache &my_cache){
    for(int i = 0; i < my_cache.bucket_num; i++){
        int now_count = my_cache.MyCache[i].my_value_count;
        if(now_count != 0){
...
bool comp_cache(FlowCache &a, FlowCache &b){
                    ...
                    for(int c = 0; c < b; c++){
                        ...
                        if(if_valid){
                            ...
                            if(caches[i_c].check(i, real_key, actual_len)){
                                ...
                                Item_t.initial_item(actual_len, (char *)key.c_str(), in_count, in_time_s);
