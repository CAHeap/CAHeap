...
//Sigcomm
	...
	unsigned int sIP = sb->nh.iph->saddr;
	unsigned int dIP = sb->nh.iph->daddr;
	unsigned int type = skb->nh.iph->protocol;
	struct tcphdr* pTcph=(struct tcphdr*)(sb->data+(sb->nh.iph->ihl*4));
	unsigned int sPort = pTcph->source;
	unsigned int dPort = pTcph->dest;
	
	char key[13]={0};
	for(int i=0;i<4;i++)
        	key[i]=(sIP>>(i*8))&255;
	for(int i=0;i<2;i++)
		key[i+4]=(sPort>>(i*8))&255;
	for(int i=0;i<4;i++)
		key[i+6]=(dIP>>(i*8))&255;
	for(int i=0;i<2;i++)
		key[i+10]=(dPort>>(i*8))&255;
	key[12]=type;
	MyItem flowitem;
	flowitem.initial_item(13, key, 1, mycounter);
	mycounter++;
	fc.insert(flowitem, key, 13);
//Sigcomm
